...
-- Secure checkout RPC: calculates totals from the database instead of trusting the browser.
...
begin
  ...
  insert into public.orders(user_id,status,payment_status,payment_method,payment_reference,subtotal,total,currency,customer_note)
  values(v_user,'pending','submitted',trim(p_payment_method),trim(p_payment_reference),v_subtotal,v_subtotal,'USD',nullif(trim(p_customer_note),''))
  returning id into v_order;

  for v_item in select * from jsonb_array_elements(p_items) loop
    v_qty := greatest(1, least(99, coalesce((v_item->>'quantity')::integer, 1)));
    select * into v_product from public.products where id = (v_item->>'product_id')::uuid and is_active = true;
    insert into public.order_items(order_id,product_id,product_title,unit_price,quantity,line_total,download_url)
    values(v_order,v_product.id,v_product.title,v_product.price,v_qty,v_product.price*v_qty,v_product.download_url);
  end loop;
