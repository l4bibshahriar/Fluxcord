...
if(chart){
  chart.addEventListener("mousemove", e=>{
    ...
    tipA.textContent = d.risk + "%";
    tipB.textContent = d.ai + "%";
    const tipC = document.getElementById("tipC");
    if (tipC) tipC.textContent = d.unemployment + "%";
    tooltip.style.left = `${Math.max(10, Math.min(rect.width - 180, x + 14))}px`;
    tooltip.style.top = `${Math.max(10, Math.min(rect.height - 130, e.clientY - rect.top - 78))}px`;
...
document.querySelectorAll('a[href*="discord.gg/9EpdA3KpQE"]').forEach(el => {
  el.addEventListener("click", e => { e.preventDefault(); redirectToDiscord(el); });
});
...
function createConfetti() {
  const box = document.querySelector(".confetti-pieces");
  if (!box) return;
  box.innerHTML = "";
  for (let i = 0; i < 42; i++) {
...
if (discountPopup) setTimeout(openDiscountPopup, 650);
