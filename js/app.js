...
async function handleRegister(e){e.preventDefault();const first=$('#firstName').value.trim(),last=$('#lastName').value.trim(),email=$('#registerEmail').value.trim(),password=$('#registerPassword').value,confirm=$('#registerConfirm').value;if(password!==confirm){toast('Passwords do not match.','error');return}const btn=e.submitter||e.target.querySelector('button[type=submit]');if(btn){btn.disabled=true;btn.textContent='Sending confirmation…'}try{const r=await fetch('/api/auth-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'register',firstName:first,lastName:last,email,password})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Could not create your account.');toast(data.message||'Account created. Check your email.','success');setTimeout(()=>location.href='login.html',1100)}catch(err){toast(err.message,'error')}finally{if(btn){btn.disabled=false;btn.textContent='Create Account'}}}
